...
import {DataType} from '../../models/Settings'
...
import { CreateSettingModel } from '../../models/Settings'; 
import Swal from 'sweetalert2'
import withReactContent from 'sweetalert2-react-content'


export class CreateSetting extends Component {
    constructor(props) {
        super(props);
        this.state = {
  ...
  static displayName = CreateSetting.name;

  async settingTypeData() {
    const api = new ApiSettingType();
    const response = await api.getSettingType();
    this.setState({settingTypes: response.body, settingType:response.body[0]})    
  }


  async createSetting() {
    const api = new ApiSetting();
    const createSetting= CreateSettingModel;
    createSetting.parameterName = this.state.parameterName;
    createSetting.parameterValue = this.state.parameterValue;
    createSetting.parameterDescription = this.state.parameterDescription;
    createSetting.dataType = this.state.enumDataType;
    createSetting.settingTypeId = this.state.settingType.id;
    const response = await api.createSetting(createSetting);
            ...
            <div className="content__wrap">
                <nav aria-label="breadcrumb">
                    <ol className="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Crear Configuración</li>
                    </ol>
                </nav>
                <h1 className="page-title mb-0 mt-2">Crear Configuración</h1>
                <p className="lead"></p>
            </div>
        </div>
